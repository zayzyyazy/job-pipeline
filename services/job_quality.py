...
def prepare_parsed_job_for_pipeline(parsed: Dict[str, Any]) -> Tuple[Dict[str, Any], Optional[str]]:
    """
    Full cleanup. Does not hard-reject: weak titles become a placeholder so jobs still ingest.
    Returns (parsed, warning_reason_or_None) for metrics only.
    """
    parsed = apply_company_from_title(parsed)
    ok, reason = assess_title_quality(parsed.get("title", ""))
    if not ok:
        er = (parsed.get("extraction_reason") or "") + f";title_softened:{reason}"
        parsed["extraction_reason"] = er.strip(";")
        prev_c = int(parsed.get("extraction_confidence") or 50)
        parsed["extraction_confidence"] = min(prev_c, 38)
        parsed["title"] = "Unknown role from job alert"
        if not (parsed.get("company") or "").strip() or str(parsed.get("company")).lower() in (
            "unknown company",
            "unknown",
        ):
