...
def _coerce_seniority(val: Any) -> str:
    raw = str(val or "").strip().lower()
    if not raw or raw == "unknown":
        return "unknown"
    compact = raw.replace("-", " ")
    if "junior" in compact or "entry" in compact or "intern" in compact:
        return "junior"
    if "senior" in compact:
        return "senior"
    if "mid" in compact or "intermediate" in compact:
        return "mid"
    if raw in ("junior", "mid", "senior"):
        return raw
    return "unknown"
...
def _apply_source_quality_guardrails(job: Dict[str, Any], out: Dict[str, Any]) -> None:
    ...
    if sq in ("email_snapshot", "not_found"):
        ...
        out["skills"] = []
        out["key_responsibilities"] = []
        out["seniority_level"] = "unknown"
...
def _dedupe_merge_lists(existing: List[str], additions: List[str]) -> List[str]:
...
def _maybe_keyword_fallback(
    ...
    if effective_sq not in ("manual_paste", "full_posting", "partial_posting"):
        return
    if len(combined_jd.strip()) <= 280:
        return
    req = list(out.get("required_skills") or [])
    tools = list(out.get("tools_technologies") or out.get("tools_or_technologies") or [])
    nice = list(out.get("nice_to_have_skills") or [])
    tf = str((job or {}).get("target_fit") or "").lower()
    if tf == "reject":
        return
    strong_medium = tf in ("strong", "medium")
    # Fill any empty bucket from JD keywords (merge, do not wipe AI-filled lists).
    needs = (not req) or (not tools) or (not nice)
    if not needs and not (strong_medium and len(req) < 2 and len(combined_jd) > 500):
        return
    fb = extract_skills_fallback(combined_jd)
    touched = False
    if fb.get("required_skills"):
        if not req:
            out["required_skills"] = list(fb["required_skills"])
            touched = True
        elif strong_medium and len(combined_jd) > 520:
            merged = _dedupe_merge_lists(req, fb["required_skills"])
            if len(merged) > len(req):
                out["required_skills"] = merged
                touched = True
    if fb.get("nice_to_have_skills") and not nice:
    ...
    if fb.get("tools_technologies"):
        if not tools:
            ...
            merged_t = _dedupe_merge_lists(tools, fb["tools_technologies"])
            if len(merged_t) > len(tools):
                out["tools_technologies"] = merged_t
                out["tools_or_technologies"] = merged_t
                touched = True
    if touched:
        out["skills"] = list(out.get("required_skills") or [])
        esu = str(out.get("enrichment_sources_used") or "").strip()
...
def _maybe_responsibilities_fallback(effective_sq: str, combined_jd: str, out: Dict[str, Any]) -> None:
    if effective_sq not in ("manual_paste", "full_posting", "partial_posting"):
        return
    if len(combined_jd.strip()) < 180:
        return
    kr = out.get("key_responsibilities")
    if kr and isinstance(kr, list) and len(kr) > 0:
        return
    items = extract_responsibilities_fallback(combined_jd)
    if not items:
        return
    out["key_responsibilities"] = items
    esu = str(out.get("enrichment_sources_used") or "").strip()
    if "responsibilities_fallback_v1" not in esu:
        out["enrichment_sources_used"] = f"{esu} | responsibilities_fallback_v1".strip()
...
class AIService:
    ...
    def _fallback(job: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "what_the_job_is": "Unknown",
            "key_responsibilities": [],
            "business_context": "Unknown",
            "why_this_role_exists": "Unknown",
    ...
    def enrich_job(self, job: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # If combined posting text is still thin, try discovery again (same helper as sync — no DB writes here).
        try:
            thin = len(_combined_jd_text(manual, discovered, posting).strip()) < 650
            if thin and settings.enable_ddg_discovery and (job.get("title") or job.get("company")):
                from services.job_discovery import discover_job_details

                boost = discover_job_details(dict(job), log_prefix="[DISCOVERY][ENRICH]", force_research=False)
                bt = (boost.get("discovered_text") or "").strip()
                bp = (boost.get("job_page_text") or "").strip()
                if len(bt) > len(discovered):
                    discovered = bt
                if len(bp) > len(posting):
                    posting = bp
        except Exception:
            pass
        ...
        if not self.enabled or not self.client:
            ...
            _maybe_keyword_fallback(effective_sq, combined_jd, out, job=job)
            _maybe_responsibilities_fallback(effective_sq, combined_jd, out)
            _finalize_skill_tool_outputs(out)
...
- manual_paste: SOURCE 0 is a full pasted JD. Populate what_the_job_is, business_context, why_this_role_exists with evidence from postings; enumerate key_responsibilities ONLY as verbatim-ish bullets taken from Sources; seniority ONLY if explicit.
- full_posting: ATS/careers-class text via SOURCE 3/2. Same rule: rich but never invented — responsibilities/skills only when enumerated in Sources.
  Explain fit vs automation / AI / product / startup workflows with evidence from posting text.
- partial_posting: Text is thinner; prefer concrete facts but allow hedging (“not stated”); shorter lists are OK — still no fabrication.
- email_snapshot OR not_found: Gmail + snippets only. You MUST treat this as UNVERIFIED. Do NOT pretend the email is the full JD.
  required_skills, nice_to_have_skills, tools_or_technologies, AND key_responsibilities MUST be EMPTY arrays unless wording is verbatim in SOURCE 1.
  what_the_job_is MUST be <=2 cautious sentences (hedge with \"may\"/\"appears\" when unsure); prefer \"Unknown\" to invention.
  business_context and why_this_role_exists MUST be short and hedged, or literal \"Unknown\".
  seniority_level MUST be \"unknown\" unless level wording is verbatim in SOURCE 1.
...
- what_the_job_is (string — role in plain language; use Unknown only if Sources truly lack clues)
- key_responsibilities (array of short strings — ONLY bullets/tasks explicitly stated in Sources; EMPTY if none)
- business_context (string — team/company/problem context grounded in Sources, else Unknown)
- why_this_role_exists (string — hiring motive if evidenced, else Unknown)
- seniority_level (exactly one of: junior | mid | senior | unknown — unknown unless Sources state level clearly)
...
""".strip()
        ...
        try:
            ...
            key_resp_lines = list(_as_list(parsed.get("key_responsibilities")))
            ...
            out = {
                ...
                "what_the_job_is": _as_text_field(parsed.get("what_the_job_is")),
                "key_responsibilities": key_resp_lines,
                "business_context": _as_text_field(parsed.get("business_context")),
                "why_this_role_exists": _as_text_field(parsed.get("why_this_role_exists")),
                "seniority_level": _coerce_seniority(parsed.get("seniority_level")),
            }
            ...
            _maybe_keyword_fallback(effective_sq, combined_jd, fallback, job=job)
            _maybe_responsibilities_fallback(effective_sq, combined_jd, fallback)
            _finalize_skill_tool_outputs(fallback)
