...
_RESP_HEADER = re.compile(
    r"(?is)(?:^|\n)\s*(?:responsibilit(?:y|ies)|key\s+responsibilit(?:y|ies)|your\s+role|main\s+tasks|"
    r"aufgaben|tätigkeiten)\s*[:\-]?\s*\n?",
)
_BULLET_LINE = re.compile(r"^\s*(?:[-*•]+\s+|\d+[.)]\s+)(.+)$")


def extract_responsibilities_fallback(text: str, max_items: int = 12, max_chars: int = 220) -> List[str]:
    """Pull bullet lines after responsibility-style headings (or early JD bullets) — substring only, no guessing."""
    if not text or not text.strip():
        return []
    window = text[:12000]
    m = _RESP_HEADER.search(window)
    block = window[m.end() :] if m else window
    ...
    for line in lines:
        ...
        if not raw_line.strip():
            if out and m:
                break
            continue
        bm = _BULLET_LINE.match(raw_line)
        if not bm:
            continue
        fragment = bm.group(1).strip()[:max_chars]
        if len(fragment) < 8:
            continue
        low = fragment.lower()
        if low.startswith(("apply now", "we offer", "benefits", "requirements", "qualifications", "nice to have")):
            break
        out.append(fragment)
