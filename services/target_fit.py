...
def _clinical_student_support(blob_lc: str, title_lc: str) -> bool:
    """Conservative detector for bedside/patient clerical STUDENT postings (not SaaS portal engineering)."""
    compact = title_lc.replace(" ", "").replace("/", "")
    if "patientensupport" in compact or "patienten-support" in title_lc.lower().replace(" ", ""):
        return True
    if "patient support" in title_lc or "patientenbetreuung" in title_lc:
        return True
    werk = ("werkstudent" in title_lc) or ("werkstudentin" in title_lc) or ("working student" in title_lc)
    if werk and ("medizin" in title_lc or "medical" in blob_lc[:2000]):
        if any(x in title_lc for x in ("support", "patient", "betreuung", "pflege", "klinik")):
            return True
    return False
...
def evaluate_target_fit(job: Dict[str, Any], email: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _clinical_student_support(text, title_lower):
        return {
            "target_fit": "reject",
            "target_score": 18,
            "matched_keywords": ["clinical_student_support_detector"],
            "reject_reason": "Patient/clinical/medical student support orientation — outside automation/AI tooling path.",
        }
